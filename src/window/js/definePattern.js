...
var bossPattern = {
    826100: {
        ...
        comment: {
            ...
            50: { title: '비고', desc: '통상 공격을 하지 않음. 마비, 수면, 공포 무효 HP 50%에서 마그나 모드로 전환' },
    ...
    getTypeByPatternPerHp: function (id, patternType, perHp) {
        ...
        // 키 역순 정렬
        let patternPerHpList = Object.keys(patternObj).reverse();
        let nowPatternPerHp = null;
        let beforePatternPerHp = null;
        for (i = 0; i < patternPerHpList.length; i++) {
            ...
            // 첫 패턴이 현재 HP보다 높으면 첫 패턴 노출
            if (i == 0 && patternPerHp < perHp) {
                nowPatternPerHp = patternPerHp;
                break;
            }

            if (perHp > beforePatternPerHp) {
                beforePatternPerHp = patternPerHp;
                continue;
            }

            if(beforePatternPerHp > perHp) {
                if(perHp > patternPerHp) {
                    nowPatternPerHp = patternPerHp;
                    break;
                }
            }
